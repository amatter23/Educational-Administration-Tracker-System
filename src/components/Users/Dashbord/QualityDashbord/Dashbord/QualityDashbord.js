...
import classes from './QualityDashbord.module.css';
    ...
    try {
      const response = getDepartmentStatics(userData.role).then(data => {
        ...
        if (data.status === 200) {
          ...
          console.log(data.result[`${userData.role.replace('_admin', '')}`]);
...
export default QualityDashbord;
